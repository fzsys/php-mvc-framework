...
<div class="vg-carcass vg-hide">
    <div class="vg-main">
        <div class="vg-sitename vg-one-of-twenty vg-firm-background-color2  vg-center">
            <a href="<?= PATH ?>" target="_blank">
                <?= end(explode('/', SITE_URL)) ?>
            </a>
        </div>
        <div class="vg-element vg-ninteen-of-twenty vg-firm-background-color4 vg-space-between  vg-box-shadow">
            ...
            <!--кнопка-->
            <a href="<?= PATH . Settings::get('routes')['admin']['alias']?>/createsitemap" class="vg-element vg-box-shadow sitemap-button">
        ...
        <div class="vg-wrap vg-firm-background-color1 vg-center vg-block vg-menu">
            <?php if($this->menu): ?>
                <?php foreach ($this->menu as $table => $item): ?>
                    <a href="<?= $this->adminPath?>show/<?= $table ?>" class="vg-wrap vg-element vg-full vg-center <?= $table === $this->table ? 'active' : '' ?>">
