...
abstract class BaseModel extends BaseModelMethods
{
    ...
    {
        // sec connection to DB
        $this->db = @new \mysqli(HOST, USER, PASS, DB_NAME);

        // if DB connection failed
        if ($this->db->connect_error) {
            throw new DbException('Ошибка подклчения к базе данных: ' . $this->db->connect_errno . ' ' . $this->db->connect_error);
        }

        // set DB connection encoding
    ...
    final public function query($query, $crud = 'r', $return_id = false)
    {
        ...
        // return result depending on CRUD option
        switch ($crud) {
            case 'r':
                if ($result->num_rows) {
                    $res = [];
     ...
     * @param array $set - parameters
     * fields => [fileld => value] - fields to insert data
     * files => [field => value] /  [field => [value]];
     * except => ['except1', 'except2'] - except this elements from query
     * return_id => true|false;
     * @return mixed
     * @throws DbException
     */
